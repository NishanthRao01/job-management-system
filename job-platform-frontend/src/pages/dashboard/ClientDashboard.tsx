...
const ClientDashboard = () => {
  ...
  const statCards = [
    { label: 'Total Applications', value: totalCount, icon: Briefcase, gradient: 'from-indigo-500 to-indigo-600', lightBg: 'bg-indigo-50' },
    { label: 'In Interview', value: statusCounts['interview'] || 0, icon: TrendingUp, gradient: 'from-amber-500 to-orange-500', lightBg: 'bg-amber-50' },
    { label: 'Offers', value: statusCounts['offer'] || 0, icon: Clock, gradient: 'from-emerald-500 to-green-600', lightBg: 'bg-emerald-50' },
    { label: 'Applied', value: statusCounts['applied'] || 0, icon: Building2, gradient: 'from-blue-500 to-cyan-600', lightBg: 'bg-blue-50' },
  ];
        ...
        <div className="divide-y divide-slate-100">
          {jobs.length === 0 ? (
            <div className="p-16 text-center">
              ...
              <p className="text-slate-500 max-w-sm mx-auto">Subscribe to a plan and your assigned associate will start applying for you!</p>
            </div>
          ) : (
              ...
              {jobs.map((job: any) => {
                ...
                return (
                  <li key={job._id} className="hover:bg-slate-50/50 transition-all duration-200">
                    <Link to={`/dashboard/client/jobs/${job._id}`} className="p-5 sm:p-6 block">
