...
import { Building2, Calendar, MessageSquare, Plus, Search, ChevronLeft, ChevronRight } from 'lucide-react';
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-4">
          <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-200 min-w-[200px]">
            <dt className="text-sm font-medium text-slate-500 truncate">Applications Managed</dt>
      ...
      <div className="bg-white shadow-sm rounded-xl border border-slate-200 overflow-hidden">
        <div className="px-6 py-5 border-b border-slate-200 bg-slate-50 flex flex-col xl:flex-row xl:items-center justify-between gap-4">
          <h3 className="text-lg font-medium leading-6 text-slate-900">Recent Applications</h3>
        ...
        <div className="divide-y divide-slate-200">
          {jobs.length === 0 ? (
            <div className="p-12 text-center text-slate-500">
              No jobs managed yet. Start applying for your clients!
            </div>
          ) : (
            <ul className="divide-y divide-slate-200">
              {jobs.map((job: any) => (
                <li key={job._id} className="hover:bg-slate-50 transition-colors">
                  <Link to={`/dashboard/associate/jobs/${job._id}`} className="p-6 block">
                    <div className="flex items-center justify-between">
                      <div className="flex flex-col space-y-1">
                        <div className="flex items-center space-x-3">
                          ...
                          <span className="text-sm font-medium text-slate-500 px-2 py-0.5 bg-slate-100 rounded">
